# CLAUDE.md — authoring guide for *The Shape of Statistical Theory*

This is a static, bookdown-style textbook about how the core ideas of theoretical statistics fit together, generated
from Markdown by a small Python build. Read this file before editing anything.

**This is an AI-generated book, and that is disclosed to the reader** — in the
`README.md` and in the preface's "This book is AI-generated" callout. The bar the
disclosure sets (well-sourced, honest about uncertainty, verified against primary
sources) is the bar the conventions below exist to hold. Keep the disclosure in
place; do not quietly drop it.

## What this book is

- **Audience.** A graduate student who has had calculus, linear algebra, and a first course in probability, and wants the load-bearing picture of theoretical statistics: what the ideas mean and when methods actually work. Concepts come first and proofs are sketched rather than marched through, so the book reads as a map of the field anchored in intuition. Teach what the reader needs and no more: where a
  concept matters and they may not have it, teach it in enough depth to reason
  correctly, and spend the word budget on the load-bearing ideas and the
  synthesis.
- **Goal.** Not encyclopedic, and not PhD-in-the-weeds. Awareness-and-synthesis
  depth. Prefer a clear through-line the reader can follow chapter to chapter,
  and be honest about what these methods do well and what is still hard — telling
  the difference is much of the skill.
...
The one drafted chapter (`content/transformer.md`) is a worked example of the
voice and every feature; imitate it until a real chapter here reads better, then
that becomes the local style reference.
...
- **Every chapter should carry its weight in callouts:** at least one Intuition
  box, and A sharper question boxes for the pointed questions a sharp reader —
  an interviewer, a collaborator, a skeptic — would actually ask.
- **Carry second-question depth.** The concise prose gives the clean *first*
  answer; the follow-up a sharp reader asks next is delivered through (1)
  A sharper question boxes and (2) short **second-layer asides** that deepen a
  claim in a sentence or two, plus the end-of-chapter quiz (see Quizzes below).
  Conciseness and depth are complementary, not opposed: add depth through these
  vehicles, never by padding the narrative. When a topic belongs to a later
  chapter, name it with a forward-pointer rather than duplicating it here.
...
- **Define jargon on first use, and add it to the glossary.** Every term or
  abbreviation is defined inline the first time it appears and added to
  `glossary.py` (see Glossary below).
- Favor adequate coverage over a strict word count, but still cut anything that
...
assets/cover.svg        GENERATED cover (also icon.svg, apple-touch-icon.png).
                        The motif is the book's visual identity; edit
                        fig_cover/fig_icon in make_figures.py, never the files.
...
1. Find the chapter's `slug` in `toc.py` (e.g. `transformer`).
...
- Prose in this book's voice, with at least one Intuition box, the
  A sharper question boxes a sharp reader would ask, jargon defined on first
  use, honest analogies, and cross-references to related chapters.
- Its contributions to the single-source modules returned **as data, not written
  ...
  for the quiz, `Term`s for the glossary, and a
  `fig_*()` function for each `##` section.

The **orchestrator** (not the agents) integrates those contributions into
`references.py`, `quizzes.py`, `glossary.py`, and
`make_figures.py`, then regenerates figures and runs the build. Keeping every
shared-module edit in one place avoids the write conflicts that parallel agents
editing the same files would otherwise hit. A section whose real figure would be
low quality may ship a `!!! figure` placeholder and be upgraded in a focused
figure pass.
...
push auto-deploys to the live site), run these verification passes, each fanned
out one agent per reference / per chapter / per figure:
...
2. **Domain-correctness review.** Fan out a skeptical reviewer per chapter to
   check the domain claims themselves — is each statement correct, current, and
   not overstated? Reviewers cite sources and rank findings by severity. Apply
   the clear factual fixes; surface judgment calls to the author.
...
Only after all passes are clean is the batch ready to commit.

## Shipping a drafted section as a PR

Once an entire section (a Part) is drafted AND the verification passes are clean
— prose, references, figures, glossary, and
quizzes integrated and the build green — ship it for review rather than
committing to `main` directly:

1. Create a feature branch (e.g. `draft-part-2`).
2. Commit the section's work with a concise message.
3. Push the branch to this book's GitHub repo (`zorian15/shape-of-statistical-theory`).
...
!!! probe "A sharper question"
    *A pointed question a sharp reader would ask.* The crisp answer.
...
The probing callout uses the `probe` type; its **title** is the per-book framing
(`!!! probe "A sharper question"`). Keep every other title matching its type.
...
This book aims to reflect the *current, load-bearing* picture, and fields move
fast. Before drafting or substantially revising a chapter, **research the present
state of the art rather than writing from memory** — names, benchmark results,
and "what is SOTA" drift, and a confidently wrong specific is worse than a hedged
general one. Fan out web research on the chapter's key claims, prefer primary
sources (the paper, not a blog about it), and note where sources disagree so the
prose can be honest about it. This matters more, not less, because the book is
AI-generated: the disclosure promises the reader a verified synthesis.
...
challenging multiple-choice questions**. The build renders it automatically after
the References; a small inline script makes it interactive (pick an option → it is
marked right or wrong, the correct answer is revealed, an explanation appears).
Questions live in `quizzes.py`, keyed by chapter slug — the single source of
truth, mirroring `references.py`. **Do not hand-write quiz HTML.**
...
* **The bar:** questions should be *hard* the way a sharp reader's question is
  hard. The distractors are the plausible **misconceptions**, and each
  `explanation` carries a **second-layer detail** — it should teach, not just
  confirm. Draw on the same gaps the A sharper question boxes and asides target,
  so the quiz reinforces the reading.
* **Balanced, non-telegraphing options.** All options must read as comparably
  detailed and confident — one short option here or there is fine, but the correct
  one must never be the consistently longest choice or a verbatim lift from the
  prose, or the answer gives itself away on shape alone. Each distractor is a
  full, plausible misconception; aim for one *tempting near-miss* per question.
  The renderer shuffles the options on load, so the `answer` index carries no
  positional signal — write the options in any order, and never write one that
  refers to another by position ("both A and B").
...
**Every term or abbreviation is defined inline the first time it appears in the
prose, and also added to `glossary.py`** — the single source of truth, mirroring
`references.py` and `quizzes.py`. The build renders the `glossary` appendix from
it automatically, alphabetically; **do not hand-write the glossary page.**
...
* Keep each definition to one or two plain sentences aimed at the book's reader
  — enough to unblock them, not an encyclopedia entry. Plain text only, same as
  quizzes (no `$...$`; phrase math in words).
...
three when the material earns it. The floor is not a license for filler: the
book's rule that "no section is filler" applies to figures too, so find the
*honest* figure for a section rather than decorating it. Captions carry an idea,
not a label.

Figures are **generated as SVG by `figures/make_figures.py`**, not drawn by hand
and not committed as binaries. Two kinds:

* **Diagrams** (concepts: a pipeline, a mechanism, a model's input/output) are
  hand-authored SVG emitted from Python string templates. A plotting library
  fights you here; raw SVG does not.
* **Plots** (anything quantitative: benchmark curves, sizes, throughput) are
...
```html
<figure>
<img src="assets/figures/example-pipeline.svg" alt="Describe the figure for a screen reader.">
...
Nothing is drafted yet except the worked example chapter
(`content/transformer.md`) — every other chapter is a stub driven by its
`outline` in `toc.py`, so the whole book is already navigable. Replace the
example Part and chapters in `toc.py` with your book's real structure, then fill
chapters in, building shared vocabulary first.
...
- At least one Intuition box and the A sharper question questions a sharp reader
  would face you with.
- Carries second-question depth: A sharper question boxes and short second-layer
  asides cover the follow-up probes, each honest about what is deferred to a
  later chapter (forward-pointer, not duplication).
...
| `DRAFT` | `True` | While true, every page carries `noindex`. Flip to `False` to let the book be found. |
...
This book was generated from the `textbook-template` copier template. When the
template's engine improves, run `copier update` in this repo to pull the changes
as a three-way merge, review the diff, and commit. Your content
(`toc.py`, `content/`, `references.py`, `quizzes.py`,
`glossary.py`, the book-specific `fig_*()` functions) is yours; the
engine (`build.py`, `style.css`, the figure infrastructure) is what updates.
